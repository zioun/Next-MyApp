...
const page = () => {
  ...
  return (
    ...
    <div className="grid grid-cols-2 gap-5">
      {blogs.map((blog) => (
        <div key={blog.id} className="border p-5">
          <div className="flex justify-between">
            <h1 className="font-bold text-lg">{blog.title}</h1>
            <button className="font-bold border px-3 bg-gray-200 rounded-full">
              <Link href={`/blogs/${blog.id}`}>View</Link>
            </button>
          </div>
          <p className="text-gray-700">{blog.content}</p>
          <div className="flex gap-5 py-3">
            <h3 className="border px-2 bg-gray-200">{blog.author}</h3>
            <p className="border px-2 bg-gray-200">{blog.category}</p>
            <data className="border px-2 bg-gray-200" value={blog.date}>
              {blog.date}
            </data>
          </div>
          <div>
          ...
          <div className="flex gap-5 mt-3">
            <p className="border px-2 bg-gray-200">Views: {blog.views}</p>
            <p className="border px-2 bg-gray-200">Likes: {blog.likes}</p>
            <p className="border px-2 bg-gray-200">Comments: {blog.comments}</p>
          </div>
        </div>
      ))}
    </div>
